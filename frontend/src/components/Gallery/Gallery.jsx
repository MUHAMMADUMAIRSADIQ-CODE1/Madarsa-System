      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
          {gallery.map((item, i) => (
            ...
            >
              <div className={`aspect-[4/3] bg-gradient-to-br ${galleryColors[i]} flex items-center justify-center`}>
              ...
              {/* Hover Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-end p-5 lg:p-6">
                <div>
                  <h3 className="text-white font-heading font-bold text-lg">{item.title}</h3>
                  <span className="text-xs text-gold-light/80 font-medium uppercase tracking-wider">{item.category}</span>
                </div>
              </div>
            </div>
          ))}
